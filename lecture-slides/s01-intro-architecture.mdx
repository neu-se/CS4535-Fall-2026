...
**Structure (Who's here → How it's graded → The vocabulary → The stack → The debt → Your slate):**
- Arc 1: Who's In the Room (~15 min) — introductions, everyone in the room, including the AI conversation
- Arc 2: How This Works (~9 min) — the four bands, what counts as evidence, the two halves of the calendar
- Arc 3: The Vocabulary (~4–8 min) — the six infrastructure building blocks, then a real slide each on "what is a database" and "why is there an EventBridge". Lifted from CS 3100 `l21-serverless`
...
**The pitch is gone, on purpose.** Everyone enrolled so far came in through an application, and all of them did the pre-application activity. They are already sold on the course, and most have already had Pawtograder running on their own laptop. Spending ten minutes telling them what the product is and why the course is interesting would be telling them what they already did. That time went to introductions instead.

**What the cut pitch slides carried that still has to be said** — say it inside your own introduction (see the Introductions slide): what Pawtograder is, that it runs ten courses this term including this one, and the blast radius. That's about two minutes of talking.
...
**The timing risk is now real.** Fifteen minutes of introductions is worth it on a room this size, but it costs time you have to find elsewhere. The deck as written runs about 74 minutes in a 65-minute room. **Pre-commit to the cuts before you walk in** rather than discovering them at 11:15. The two designated cuts, in order, buy back six minutes and cost almost nothing (see the cut order below).

**Arc 3 is the other variable.** Everyone enrolled so far set up local Supabase and Postgres for the pre-application activity, so the floor is higher than it would be in an open-enrollment section. This arc is likelier to take four minutes than twelve. But *ask* rather than assume. The introductions will already have told you who took 3100 with you and who's coming back from co-op, so you'll know before you get here.
...
**Cut order beyond that**, in this sequence: drop the "Data Access: Prefer RPCs" slide, then shorten the debt talk over the complexity map to two bullets. **Protect Arc 6** — bids are due 9/17 and this is the only session that explains the areas of the system.

**Do not buy time back out of the introductions.** On a roster this small, this is the session that decides whether the room talks to each other for the next fourteen weeks. If you're behind at 11:00, take it out of Arc 4, which is over-provisioned for exactly this reason.
...
**The anti-intimidation thread runs through the whole deck**, deliberately, in four places: the fragment on the learning-objectives slide ("describe" and "map" are the verbs), the small right-aligned `full session: <date>` pointers under the technical slides, the roadmap table at the end of Arc 4, and the "nobody has read all of it" line, which now lives in your own introduction rather than on a slide. If you only land one, land the roadmap.
...
**Say that in one sentence and move.** Introductions are two slides away, and your own introduction is where you explain the product and its users.
...
**Time allocation:**
- Arc 1: Who's In the Room (~15 min)
- Arc 2: How This Works (~9 min)
- Arc 3: The Vocabulary (~4–8 min) — prerequisite for objective 1
...
**Introductions aren't on the objectives list and that's fine** — don't apologize for them or promise they'll be quick. On a roster this small, knowing who everyone is matters more than anything else on the list.

> **Transition:** Before any of that — it's a small room in here. Let's find out who.
</aside>
...
{/* ============================================ */}
{/* ARC 1: WHO'S IN THE ROOM (~15 min)           */}
{/* Nine of us. This is the whole team.          */}
{/* ============================================ */}
...
<div style={{fontSize: '0.75em', textAlign: 'left', marginTop: '0.6em'}}>

Tell us:

<ol>
  <li>Your name, and something you did over the summer</li>
  <li>What you want out of this Fall &mdash; in this course, and after it</li>
  <li>Your relationship with AI tooling: <strong>how you use it, how you <em>want</em> to use it, and how you don't</strong></li>
</ol>

</div>

<p style={{fontSize: '0.75em', marginTop: '0.5em', fontStyle: 'italic'}}>
I'll go first.
</p>

<div className="fragment" style={{backgroundColor: 'rgba(42,157,143,0.12)', padding: '0.5em', borderRadius: '8px', fontSize: '0.72em', marginTop: '0.6em'}}>
...
<aside className="notes">
**Fifteen minutes, and protect them.** Everyone enrolled, plus you. Roughly 90 seconds each, and you should be the one who models the length — if your own introduction runs five minutes, theirs will too.

**Go first, and use your turn to carry what the cut pitch slides used to say.** Four things, in about two minutes:
- Who you are and what you work on.
- **What Pawtograder is and how long it's existed** — say roughly how many students have been graded through it. That number is the credibility hook for the whole semester.
- **Who's using it right now:** CS 2000, 2100, 3100, 3650, 4530, 5001, 5004, 5008, 5010 — *and this course*. They'll submit this course's work through the system they're about to change. Break a student view and you've annoyed several thousand people across ten courses. Break an instructor grading view and a course can't return grades that week. Break it in December and you've done it during finals.
- **The scale, honestly:** about 300k lines of TypeScript, 417 migrations, 62 edge functions, 129 routes — and zero people who have read all of it, including me. Nobody reads it all. You read the part you own and you learn how to find the rest.

**Then answer your own question 3 before you ask it of anyone else.** They will calibrate to whatever you say, so say the real thing, including the parts where you don't reach for it.

**Why question 3 is in the introductions and not on a policy slide.** "Use anything" is a permission and nothing more, and a slide announcing it teaches nothing. This room has to compare notes for fourteen weeks, so the first data point is everyone in it saying out loud what they currently do. Expect real spread: someone drives an agent for everything, someone has decided it makes them worse, and both are welcome. **Don't adjudicate any of the answers.** Just note where the room is.

**What you owe them in return, said once at the end of the round** — "use anything" is an abdication if we stop there, so name the three things we actually provide:
- **Instruction:** the agents and workflows session, Wed 10/7 (s12) — structuring context, decomposing across sub-agents, judging when an agent is the wrong tool.
- **A forum:** a standing channel for prompts, setups, and failures. Treat it as a lab notebook.
- **Clinic:** bring agent output you're unsure about. "Should I trust this?" is a legitimate use of clinic time.

**The practical hook, worth 20 seconds:** the repo ships `CLAUDE.md` and `AGENTS.md`. Students who point their agent at those two files get dramatically better results than students who don't — good context beats good prompting, and it's the first concrete lesson in this course's AI posture.

**Then the accountability line — don't soften it:**

> "The agent wrote it" isn't a defense at a demo day. You shipped it.

**Listen for two things you need later today:**
- **Who took CS 3100 with you, and who's back from co-op or coming in from elsewhere.** That answer sets the length of Arc 3 — four minutes or twelve — and you'd otherwise have to stop and ask for it there.
- **Where each person's energy is**, such as users and design, infrastructure, data, or product. You're announcing teams on 9/24 off bids you haven't seen yet, and this round is the only unfiltered version of that signal you'll get.

**Close the round by saying the roster is still open**, because this is the one moment where it lands. Admissions are rolling, there are seats, and the people best placed to fill them are sitting in front of you — they know who in their year would be good at this and would never have heard of the course otherwise. Ask them directly to send someone to the apply page. Two framings that both work: a small team is an advantage and you'd still take a few more, or simply that you'd rather they picked the next teammates than that a seat went empty.

**Do not put a headcount on a slide, here or anywhere.** The number moves, and a student who hears "there are eight of us" on day one reads a later arrival as a change of plan rather than the normal thing it is.

**TODO:** Name the actual forum channel once Discord is set up. Also decide whether staff introduce themselves here or at Thursday's working session, where they'll actually be floating.

**Deliberate contrast with CS 3100**, if it comes up. AI is restricted there in weeks 1–5 because the skill being built is the typing. Here the skill is judgment, integration, and accountability — and the agent doesn't change any of those.

> **Transition:** That's the room. Here's how the room gets graded.
</aside>
...
{/* ============================================ */}
{/* ARC 2: HOW THIS WORKS (~9 min)               */}
{/* Grading, evidence, and the two halves        */}
{/* ============================================ */}
...
**The line that used to live on the cut "Deal" slide, and is worth keeping somewhere:** I define the minimum for an A; you decide the minimum for your goals. It'll be tested in week 3 when someone asks "is this enough?" The answer is: enough for what you said you wanted — which is why you just told the room what you want out of the semester.

> **Transition:** So what do you actually hand in? Almost nothing.
</aside>
...
**Say the team arithmetic here rather than making them wait for Arc 6:** on today's roster that's two teams, and two of the seven projects. It changes how they listen to the slate and it stops one person deciding they've already got a team of one. Say "on today's roster" and mean it — the number moves if late enrollments land, and you don't want to walk that back on Sep 24.

> **Transition:** That's the rhythm. Before the map, ninety seconds on what the pieces of a map even are.
</aside>

</Slide>

{/* ============================================ */}
{/* ARC 3: THE VOCABULARY (~4-8 min)             */}
...
**Do not lecture the grid — name each box in one sentence and move.** Four to eight minutes for the whole arc, and with this cohort it's likely the short end. Every one of them stood a Postgres and a Supabase stack up on their own laptop for the pre-application activity, so "database" and "object storage" are not abstractions to them.
...
**You already know who's in front of you** from the introductions. Set the pace from that rather than asking again here.
...
<p style={{fontSize: '0.58em', marginTop: '0.3em', textAlign: 'right', fontStyle: 'italic', color: '#4A90A4'}}>
CI in depth: <strong>Wed 9/23</strong> · what it tests: <strong>Thu 9/24</strong>
</p>
...
**Do not** get into TableController here. It gets a whole session on 9/16, Architecture III: The Client, which is where the frontend conventions live.
...
| The frontend: App Router, types, TableController | **Wed 9/16** — Architecture III: The Client |
| Migrations, down paths, rollback | **Thu 9/17** — Architecture IV: Deployment |
| CI/CD, feature flags, dark launches, trunk-based development | **Wed 9/23** — Software Processes & Continuous Delivery |
| E2E and visual acceptance testing | **Thu 9/24** — Testing |
| IaC, drift, Sentry/Grafana, incident response | **Thu 10/1** — Tech Talk: Operations |
...
**Give each project about 60 seconds and do not improvise.** Say the arithmetic out loud before you start: today's roster is two teams, so **two of these seven run and five don't**. If you skip that, someone spends the week falling in love with a project that never happens. The bid is what decides which two survive. The honest pitch for each:
...
**Students bid far more sensibly once they know how many projects can actually run**, and saying so now is kinder than letting them discover it on Sep 24. It also changes what a bid *is*: with two teams, their second and third choices are load-bearing.

**One upside of a small roster worth naming:** you can build the teams around what people said they wanted this morning. Say that you'll do it — it makes the bids more honest.
...
<aside className="notes">
**Do not turn this into an overnight assignment.** Most of the room stood this stack up once already for the application, so the instinct is to ask them to do it again tonight. Resist it. The ask is that they turn up *able* to do it: a laptop, Node, Docker installed. The setup itself is what Thursday is for, and a student who spent last night fighting Docker alone is worse off than one who arrives fresh and does it with help in the room.

**Say the reason out loud, because it changes what they hear.** Installing Docker takes an hour of downloading and no thinking. Doing it in a room with staff and everyone else takes twenty minutes and teaches them who to ask. The only thing that can't happen in the room is the download.

**The one thing worth doing beforehand, if they have time:** a fresh clone. `main` has moved since they applied, the pinned Supabase CLI version matters, and stale Docker volumes are the most common way a setup that worked in August stops working in September. Frame it as a head start, not a requirement.
...
**The "note one thing that's wrong" bullet is optional on purpose.** For anyone who does get there, it leads into the ticket hunt, due 9/16, and into a bid grounded in something they actually saw. Nobody is behind for skipping it.
