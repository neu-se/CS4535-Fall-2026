...
These rules apply to all prose in this repository. They were generated by `/ai-slop:init` from the [ai-slop-skill](https://github.com/se-uhd/ai-slop-skill) (skill version 2026-07_rev2) and combine the writing rules maintained by the [Software Engineering Group at Heidelberg University](https://github.com/se-uhd) with a general AI-trope catalog from [tropes.fyi](https://tropes.fyi) by [ossama.is](https://ossama.is).
...
- **Use "such as" not "like" to introduce examples.** Write "scenarios such as code completion," not "scenarios like code completion." `Like` is colloquial in this construction; `such as` is the more formal register. Apply this even when the original prose already uses `like` — do not carry it over verbatim into a rewrite. `Like` remains correct as a verb ("readers like terse prose") and in deliberate similes. Course handouts are the exception: their register is conversational, and `like` introducing a quick example ("units like pinch or dash") is fine there.
...
- **No excessive bold or formatting.** Do not bold phrases for emphasis in running text. Reserve bold for subsection-level headers or matching the document's existing conventions. In course handouts, follow **Bold labels, not sentences** under *Course handout voice*.
...
## Course handout voice

Everything under `docs/`, plus `src/pages/syllabus.md`, is something a student reads to find out what to do. The reference for tone is the CS 3100 handouts in `../cs3100-public-resources/assignments/`; read one before drafting. The rules below are the differences that keep showing up when a handout is drafted from scratch instead of adapted.

Two exceptions. `src/pages/apply.md` is a recruiting page where persuasion is the job, and it is exempt from this section. The decks in `lecture-slides/` are speaker notes: the register rules apply to them, since prose written to be spoken should read the way it will be said, but their aphoristic cadence ("that's the whole concept") is a delivery device rather than a tell, and it stays.

- **Use contractions.** Write "you'll", "don't", "isn't", "we're". The 3100 handouts prefer them roughly two to one; the uncontracted forms ("you will", "do not", "it is") accumulate into a declamatory register that doesn't sound like a person explaining an assignment.
- **Use the standard section headings.** Overview, Learning Outcomes, Instructions (or What You Hand In), Technical Specifications, AI Policy, Grading Rubric, Submission, Dates. A student skimming for the rubric should find it under the same heading in every handout. Don't swap them for rhetorical headings ("Why you are doing this", "Why this one is different", "Use AI, own the result"); the motivation goes in a paragraph under Overview.
- **State the requirement, then the reason, in the same register.** The reason is a sentence of explanation, not a moral. Cut aphoristic closers ("That is the whole skill", "The point was never the diff", "Finding work is the half of engineering that nobody teaches"), pronouncements about the industry or the student's career, and any sentence whose only job is to make the assignment feel important.
- **Don't count the deliverables in a fragment before listing them.** "Four artifacts. Two tickets, two triage verdicts." and "Two things stay yours:" announce a list that the list itself already announces. Introduce it with a full sentence, or just list it.
- **Don't frame a requirement as a reversal.** "The verdict is not the deliverable. The reasoning is." states a requirement by first denying something else, which is the negative-parallelism trope in handout clothing. Say what's required. A single negation heading off a predictable misreading is fine ("a size estimate is not a commitment"); the reveal pattern is not.
- **Bold labels, not sentences.** Bold belongs on metadata labels (**Due:**, **Prerequisites:**), defined terms at first use, and the one word a requirement turns on ("**all** team members must be present"). What this rule forbids is bolding a whole sentence mid-paragraph for emphasis, and italicizing a whole sentence for drama. Two things it does not forbid: a bold run-in label opening a paragraph or list item, which is a formatting convention governed by **No excessive bold or formatting** above and should not be counted twice; and single-word contrastive italics (*is*, *before*, *same*), which the 3100 handouts use freely.
- **Put the grading contract in a table.** Components with points and criteria, or expectations with standards; dates in a table too. Prose about how much something matters is not a rubric.
- **Address the student as "you" and the course staff as "we".** Plain imperatives are right ("Schedule and hold your first team meeting"). Predictions about what the student will feel, discover, or become are not.
...
23. **Handout register.** In a `docs/` handout, confirm contractions are used, the section headings are the standard set, and the grading contract and dates are in tables rather than prose.
24. **Handout rhetoric.** Scan a handout for aphoristic closers, count-opener fragments ("Four artifacts."), reversal framing ("X is not the deliverable. Y is."), sentences bolded mid-paragraph for emphasis or italicized for drama, and claims about the student's future career. Rewrite each as a plain statement of the requirement and its reason.
25. **Example/restatement connectives.** For each em-dash or colon flagged in #2/#3, check what it introduces before collapsing it to a period or comma: if an example, prefer such as / e.g. / for example / including; if a restatement or definition, prefer i.e. / that is / in other words / namely. Name the relation when the mark is doing that work; leave payoff-clause and true-list marks alone, and keep the example signal consistent per #18.
