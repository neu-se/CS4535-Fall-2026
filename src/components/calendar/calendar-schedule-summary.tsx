...
function getLectureUrl(uid: string | undefined): string | null {
...
function TimelineEventBlock({
  ...
  const lectureUrl = getLectureUrl(event.uid);
  const isLecture = !!lectureUrl;
  ...
  const padding = isVeryShort ? 0.5 : isShort ? 1 : 1.5;
  ...
  return (
    ...
    >
      ...
      {isVeryShort ? (
        isLecture ? (
          <DocusaurusLink to={lectureUrl} style={{ textDecoration: 'none', width: '100%' }}>
            <Text
              fontSize="xs"
              fontWeight="medium"
              overflow="hidden"
...
function CompactTimelineEventBlock({
  ...
  const padding = isVeryShort ? 0.5 : isShort ? 0.75 : 1;
            ...
            events.map((event) => {
              ...
              return (
                <Box
                  ...
                  fontSize="2xs"
                >
                  {getLectureUrl(event.uid) ? (
                    <DocusaurusLink to={getLectureUrl(event.uid)!} style={{ textDecoration: 'none' }}>
                      <Text fontWeight="medium" fontSize="2xs" lineClamp={1} mb={0.25} color="blue.500" _hover={{ textDecoration: "underline" }} lineHeight="1.0">
                        {event.title}
                      </Text>
                    </DocusaurusLink>
                  ) : (
                    <Text fontWeight="medium" fontSize="2xs" lineClamp={1} mb={0.25} lineHeight="1.0">
                      {event.title}
                    </Text>
                  )}
                  {event.organizer_name && event.uid?.startsWith("lab-meeting-") && (
                    <Text fontSize="2xs" color="fg.muted" lineClamp={1} mb={0.25} lineHeight="1.0">
                      👤 {event.organizer_name}
                    </Text>
                  )}
                  <Text fontSize="2xs" color="fg.muted" lineHeight="1.0">
                    {format(start, "h:mm a")} - {format(end, "h:mm a")}
                  </Text>
                  {event.location && (
                    <Text fontSize="2xs" color="fg.muted" mt={0.25} lineClamp={1} lineHeight="1.0">
                      📍 {event.location}
                    </Text>
                  )}
                  {event.queue_name && (
